...
class QuestionListGetter {
  ...
  Map<String, List<String>> mappings =
      Map<String, List<String>>(); // Maps category name to question IDs

  static final QuestionListGetter _instance =
      QuestionListGetter._privateConstructor();
  ...
  Future<List<String>> getCategoryNames() async {
    ...
    for (QuestionCategory cat in categories) names.add(cat.name);

    return names;
  }

  QuestionCategory getCategoryByName(String name) {
    for (QuestionCategory cat in categories) if (name == cat.name) return cat;
